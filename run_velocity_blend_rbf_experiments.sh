...
# Velocity Blending (RBF) Long Video Generation Experiments
# GPU 3번에서 prompts_30.txt의 프롬프트들로 순차적으로 실험 진행
# launch.json의 "[Tweedie Caching] Velocity Blending (rbf) (GPU 3)" 설정 기반
...
VELOCITY_BLEND="--velocity_blend"  # Velocity blending 사용
BLEND_METHOD="rbf"  # RBF blending method

# 프롬프트 파일 경로
PROMPTS_FILE="prompts_30.txt"

# 출력 디렉토리
OUTPUT_DIR="./output_velocity_blend_rbf_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$OUTPUT_DIR"
...
echo "==========================================" | tee -a "$LOG_FILE"
echo "Velocity Blending (RBF) Long Video Generation Experiments Started" | tee -a "$LOG_FILE"
echo "Date: $(date)" | tee -a "$LOG_FILE"
echo "GPU: $GPU_ID" | tee -a "$LOG_FILE"
echo "Method: Tweedie Caching + Velocity Blending (RBF)" | tee -a "$LOG_FILE"
...
echo "Blend Method: $BLEND_METHOD" | tee -a "$LOG_FILE"
