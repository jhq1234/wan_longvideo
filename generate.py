...
def _validate_args(args):
    ...
    # Long video task validation
    if "long" in args.task:
        assert (args.long_window_size - 1) % 4 == 0, (
            f"--long_window_size must satisfy 4n+1, got {args.long_window_size}")
        assert args.long_multiplier >= 1, (
            f"--long_multiplier must be >= 1, got {args.long_multiplier}")
        assert 0 < args.long_overlap_start < args.long_window_size, (
            f"--long_overlap_start must be in (0, window_size), "
            f"got {args.long_overlap_start} (window_size={args.long_window_size})")
...
def _parse_args():
    ...
    # ------------------------------------------------------------------ #
    # Long video (SyncTweedies) arguments                                  #
    # ------------------------------------------------------------------ #
    parser.add_argument(
        "--long_window_size",
        type=int,
        default=81,
        help="[long video] Number of frames per temporal window (model native limit). "
             "Must satisfy 4n+1.")
    parser.add_argument(
        "--long_multiplier",
        type=int,
        default=2,
        help="[long video] Number of chunks to generate. "
             "total_frames = window_size + (multiplier - 1) * (window_size - overlap_start).")
    parser.add_argument(
        "--long_overlap_start",
        type=int,
        default=40,
        help="[long video] Frame index within a chunk where overlap starts (0-indexed). "
             "Must be < window_size.")
    parser.add_argument(
        "--long_steps",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=999,
        help="[long video] Starting timestep for outer annealing (0–1000).")
    parser.add_argument(
        "--long_t_min",
        type=int,
        default=200,
...
def generate(args):
    ...
    if "long" in args.task:
        ...
        logging.info("Creating WanT2VLong pipeline.")
        wan_long = wan.WanT2VLong(
            config=cfg,
        ...
        # Calculate total_frames for logging
        total_frames = args.long_window_size + (args.long_multiplier - 1) * (
            args.long_window_size - args.long_overlap_start)
        logging.info(
            f"Generating long video "
            f"(window_size={args.long_window_size}, "
